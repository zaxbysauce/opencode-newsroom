 ...
 *
 * Enhances session compaction by injecting newsroom context from plan.md and context.md.
...
import { readNewsroomFileAsync, safeHook } from './utils';
		...
		'experimental.session.compacting': safeHook(
			async (
				...
				if (plan && plan.migration_status !== 'migration_failed') {
					...
					if (currentPhase) {
						output.context.push(`[NEWSROOM PLAN] ${currentPhase}`);
					}
					const incompleteTasks = extractIncompleteTasksFromPlan(plan);
					if (incompleteTasks) {
						output.context.push(`[NEWSROOM TASKS] ${incompleteTasks}`);
					}
				} else {
					// Legacy fallback
				...
				if (contextContent) {
					...
					if (decisionsSummary) {
						output.context.push(`[NEWSROOM DECISIONS] ${decisionsSummary}`);
					}
				}
				...
				if (contextContent) {
					...
					if (patterns) {
						output.context.push(`[NEWSROOM PATTERNS] ${patterns}`);
					}
				}
